...
if [[ "${SKIP_RUST:-0}" != "1" ]]; then
    # RUSTUP_TOOLCHAIN: force rustup shim ke toolchain yang benar
    # even if rustup override set tidak berlaku dari build dir.
...
# Tools moved to src/uml-tools/ (Rust workspace).
# Build with cargo if available, otherwise use uml-utilities from apt.
...
# Try cargo build (Rust tools)
if command -v cargo >/dev/null 2>&1; then
    log "  Building Rust tools with cargo..."
    (
        cd "$HELPERS_SRC"
        cargo build --release --target aarch64-unknown-linux-gnu 2>&1 |             sed "s/^/  [cargo] /" || true
        CARGO_OUT="$HELPERS_SRC/target/aarch64-unknown-linux-gnu/release"
        for bin in port_helper uml_watchdog uml_mconsole uml_mkcow tunctl uml_switch; do
            [ -f "$CARGO_OUT/$bin" ] && cp "$CARGO_OUT/$bin" "$HELPERS_BUILD/" &&                 log "  → $bin ($(du -sh "$HELPERS_BUILD/$bin" | cut -f1))"
        done
    ) || log "  WARNING: cargo build failed"
fi

# uml_switch: always try apt as fallback / supplement
